...
public class Smsc {
	...
	public void doLoopback(SubmitSM smppmsg) throws InboundQueueFullException {
		...
		for(int i = 0; i < smppmsg.getOptionals().size(); i++) {
			newMessage.addVsop(smppmsg.getOptionals().get(i).getTag(), smppmsg.getOptionals().get(i).getLen(), smppmsg.getOptionals().get(i).getValue());
		}
